...
(* Build the formula e obtained from f by replacing [x; y] with [x$; y$] *)
let e =
  (* Create three integer constants named "x", "y", "xp" and "xp". *)
  let x = Arithmetic.Integer.mk_const_s ctx "x"
  and y = Arithmetic.Integer.mk_const_s ctx "y"
  and xp = Arithmetic.Integer.mk_const_s ctx ("x" ^ "$")
  and yp = Arithmetic.Integer.mk_const_s ctx ("y" ^ "$")
  in
  Expr.substitute f [x; y] [xp ; yp]

(* Display the formula e. *)
let _ =
  Format.printf "@[Formula e:@ %s@]@." (Expr.to_string e)

(* Build the formula fp obtained from f by replacing x with z * t. *)
let fp =
  (* Create three integer constants named "x", "z" and "t". *)
  let x = Arithmetic.Integer.mk_const_s ctx "x"
  and z = Arithmetic.Integer.mk_const_s ctx "z"
  and t = Arithmetic.Integer.mk_const_s ctx "t"
  in
  (* Create the expression z * t. *)
  let z_plus_t = Arithmetic.mk_mul ctx [z ; t]
  in
  Expr.substitute_one f x z_plus_t

(* Display the formula fp. *)
let _ =
  Format.printf "@[Formula fp:@ %s@]@." (Expr.to_string fp)
