...
(**
   Program automata are directed graphs with edges labeled by commands.  The
   type of nodes is made explicit in order to facilitate the implementation of
   global bounded model checking (see {! GlobalBMC}).
 *)
include DiGraph.S with module Label = Command and type Node.t = string
